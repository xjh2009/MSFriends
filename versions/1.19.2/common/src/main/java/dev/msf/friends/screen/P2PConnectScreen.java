...
/**
 * P2P connection screen for MC 1.19.2 Yarn.
 * Uses old ButtonWidget constructor API.
 */
public class P2PConnectScreen extends Screen {
    ...
    private P2PConnectScreen(@Nullable Screen parent, String peerPmid) {
        super(Text.literal("")); // 1.19.2: no GameNarrator.NO_TITLE
        this.parent = parent;
    ...
    private void connect(net.minecraft.client.MinecraftClient minecraft) {
        ...
        if (client == null || client.p2p() == null) {
            minecraft.setScreen(new DisconnectedScreen(this.parent,
                    Text.translatable("connect.failed"), FAILURE_GENERIC));
            return;
        }
        ...
        client.p2p().joinPlayer(this.peerPmid).whenComplete((result, error) -> minecraft.execute(() -> {
            if (!this.cancelled && minecraft.currentScreen == this) {
                if (error != null) {
                    ...
                    minecraft.setScreen(new DisconnectedScreen(this.parent,
                            Text.translatable("connect.failed"), reasonFor(error)));
                } else {
    ...
    @Override
    protected void init() {
        // 1.19.2 Yarn: old ButtonWidget constructor
        this.addDrawableChild(new ButtonWidget(this.width / 2 - 100, this.height / 4 + 120 + 12, 200, 20,
                Text.translatable("gui.cancel"), btn -> {
            this.cancelled = true;
            var client = MsfFriendsBoot.get();
            if (client != null && client.p2p() != null) {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float a) {
        ...
        drawCenteredText(matrixStack, this.textRenderer, this.status, this.width / 2, this.height / 2 - 50, -1);
