 ...
 *
     * <p>1.13.2 MCP Toast API: single {@code draw(ToastGui, long)} method returning Visibility.
 */
public class FriendToast implements IToast {
    ...
    @Override
    public Visibility draw(ToastGui toastGui, long startTime) {
        ...
        AbstractGui.blit(0, 0, 0, 0, TOAST_WIDTH, TOAST_HEIGHT, TOAST_WIDTH, TOAST_HEIGHT, 256, 256);
    ...
    private void renderFace(Minecraft mc, ResourceLocation skinLoc, int x, int y) {
        ...
        int s = FACE_SIZE;
        // Base head layer
        AbstractGui.blit(x, y, 8, 8, 8, 8, s, s, 64, 64);
        // Overlay head layer
        AbstractGui.blit(x, y, 40, 8, 8, 8, s, s, 64, 64);
