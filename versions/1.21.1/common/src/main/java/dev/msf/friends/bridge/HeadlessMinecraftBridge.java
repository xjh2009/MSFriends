...
public final class HeadlessMinecraftBridge implements MinecraftBridge {
    ...
    @Override public boolean inLevel() {
        try {
            ...
            java.lang.reflect.Field f = FabricReflect.mcField(mc, "level");
    ...
    public static Optional<Snapshot> tryFetch() {
        ...
        try {
            ...
            if (user == null) {
                LOGGER.info("[bridge] getUser() returned null (not logged in yet)");
                return Optional.empty();
            }
            String token = invokeUserMethod(user, "getAccessToken");
            String name  = invokeUserMethod(user, "getName");
            // getProfileId() returns UUID directly
            UUID profileId = null;
            try {
                Method getProfileId = FabricReflect.mcMethod(user.getClass(), "getProfileId");
                Object idObj = getProfileId.invoke(user);
                if (idObj instanceof UUID u) profileId = u;
                else if (idObj != null) profileId = UUID.fromString(idObj.toString());
            } catch (NoSuchMethodException e) {
                LOGGER.debug("[bridge] getProfileId() not found, trying uuid field");
                try {
                    Field uuidField = FabricReflect.mcField(user.getClass(), "uuid");
                    uuidField.setAccessible(true);
                    Object uuidObj = uuidField.get(user);
                    if (uuidObj instanceof UUID u) profileId = u;
                    else if (uuidObj != null) profileId = UUID.fromString(uuidObj.toString());
                } catch (NoSuchFieldException e2) {
                    // Last resort: try name-based lookup on fields
                    try {
                        Field uuidField = user.getClass().getDeclaredField("field_1985");
                        uuidField.setAccessible(true);
                        Object uuidObj = uuidField.get(user);
                        if (uuidObj instanceof UUID u) profileId = u;
                        else if (uuidObj != null) profileId = UUID.fromString(uuidObj.toString());
    ...
    private static Object findZeroArg(Class<?> declaring, Object target, String returnTypeSimpleName) throws Exception {
        for (var m : declaring.getMethods()) {
            if (m.getParameterCount() == 0) {
                ...
                // Match both Mojang names (dev) and intermediary names (runtime)
    ...
    /** Check if a class simple name (e.g. "class_320") corresponds to a known Mojang name (e.g. "User"). */
    private static boolean matchesIntermediaryClass(String simpleName, String mojangName) {
        // Build reverse map: Mojang simple name → intermediary simple name
    ...
    /**
     * Invoke a method on a User-like object by its Mojang name, resolving
     * to the correct runtime (intermediary) method name automatically.
     */
    private static String invokeUserMethod(Object user, String mojangMethod) throws Exception {
        try {
            ...
            // Fallback to field access (e.g. method getName() → field name)
            try {
                Field f = FabricReflect.mcField(user.getClass(), mojangMethod.replace("get", "").substring(0, 1).toLowerCase() + mojangMethod.replace("get", "").substring(1));
                f.setAccessible(true);
                Object v = f.get(user);
                return v != null ? v.toString() : null;
            } catch (NoSuchFieldException e2) {
