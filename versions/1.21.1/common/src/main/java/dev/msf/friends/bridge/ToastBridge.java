    ...
    private static Component getTitleForType(String type) {
        return switch (type) {
            case "friend.added",
                 ...
                 "friend.status.server" -> Component.translatable("toast.msf_friends." + type + ".title");
            default -> Component.translatable("toast.msf_friends.default.title");
        };
    }
    ...
    private static Component getDescriptionForType(String type, String name) {
        return switch (type) {
            case "friend.added",
                 ...
                 "friend.status.hosting",
                 "friend.status.realms",
                 "friend.status.server" -> Component.translatable("toast.msf_friends." + type + ".description", name);
            case "friend.action.rate_limited",
                 "friend.action.unknown_profile",
                 "friend.action.forbidden",
                 "friend.action.unavailable",
                 "friend.action.failed" -> Component.translatable("toast.msf_friends." + type + ".description");
            default -> Component.translatable("toast.msf_friends.default.description", name);
        };
    }
