import java.io.BufferedReader
import java.io.InputStreamReader
import java.util.zip.GZIPInputStream

plugins {
    alias(libs.plugins.forge.gradle)
}

description = "MSF Forge 1.18.2 entry point"

minecraft {
    useDefaultAccessTransformer()
}

// Pin dynamic versions used by Forge 1.18.2 FML transitive deps to avoid
// failures when maven.neoforged.net is unreachable.
configurations.configureEach {
    resolutionStrategy.eachDependency {
        when {
            requested.group == "cpw.mods" && requested.name == "modlauncher" -> useVersion("9.0.24")
            requested.group == "cpw.mods" && requested.name == "securejarhandler" -> useVersion("0.9.61")
            requested.group == "net.minecraftforge" && requested.name == "forgespi" -> useVersion("6.0.0")
            requested.group == "net.minecraftforge" && requested.name == "coremods" -> useVersion("5.0.1")
            requested.group == "net.minecraftforge" && requested.name == "eventbus" -> useVersion("5.0.7")
            requested.group == "org.apache.logging.log4j" && requested.name == "log4j-api" -> useVersion("2.17.0")
            requested.group == "org.apache.logging.log4j" && requested.name == "log4j-core" -> useVersion("2.17.0")
        }
    }
}

dependencies {
    implementation(project(":versions:1.18.2:common"))
    implementation(minecraft.dependency("net.minecraftforge:forge:1.18.2-40.2.0"))

    implementation("net.minecraftforge:javafmllanguage:1.18.2-40.2.0")
    implementation("net.minecraftforge:fmlloader:1.18.2-40.2.0")
    implementation("net.minecraftforge:fmlcore:1.18.2-40.2.0")

    implementation(libs.webrtc.java)
}

tasks.named<ProcessResources>("processResources") {
    inputs.property("version", project.version)
    val safeVersion = project.version.toString().replace("+", "-")
    filesMatching("META-INF/mods.toml") { expand("version" to safeVersion) }
}

val forgeManifestAttrs = mapOf(
    "Specification-Title" to "MSF-forge-1.18.2",
    "Specification-Version" to project.version,
    "Implementation-Title" to "MSF-forge-1.18.2",
    "Implementation-Version" to project.version,
    "MixinConfigs" to "msf-friends.mixins.json",
    "FMLModType" to "MOD"
)

tasks.named<Jar>("jar") {
    manifest { attributes(forgeManifestAttrs) }
}

tasks.register<Jar>("fatJar") {
    group = "build"
    description = "Assembles a deployable fat jar containing all module classes"
    archiveClassifier.set("all")
    duplicatesStrategy = DuplicatesStrategy.EXCLUDE
    manifest { attributes(forgeManifestAttrs) }
    from(sourceSets.main.get().output)
    val commonJar = project(":common").tasks.named<Jar>("jar")
    val verCommonJar = project(":versions:1.18.2:common").tasks.named<Jar>("jar")
    dependsOn(commonJar, verCommonJar)
    from(commonJar.map { zipTree(it.archiveFile) }) { exclude("META-INF/MANIFEST.MF") }
    from(verCommonJar.map { zipTree(it.archiveFile) }) { exclude("META-INF/MANIFEST.MF") }

    from({ configurations.runtimeClasspath.get().filter { it.name.contains("webrtc-java") }.map { zipTree(it) } }) {
        exclude("META-INF/MANIFEST.MF", "module-info.class")
    }

    exclude("com/mojang/**")
}

tasks.named("assemble") { dependsOn("relocateFatJar") }

tasks.register("relocateFatJar") {
    group = "build"
    description = "Rebuilds the fat jar with com.mojang classes relocated and Fabric intermediary names stripped"
    dependsOn("fatJar")

    val fatJarTask = tasks.named<Jar>("fatJar")
    val inputJarFile = fatJarTask.flatMap { it.archiveFile }
    val outputJarFile = inputJarFile

    inputs.file(inputJarFile)
    outputs.file(outputJarFile)

    doLast {
        val srcPkg = "com/mojang/authlib/yggdrasil"
        val dstPkg = "dev/msf/friends/shaded/com/mojang/authlib/yggdrasil"

        val authlibCoreSimpleNames = setOf(
            "YggdrasilAuthenticationService",
            "YggdrasilEnvironment",
            "YggdrasilMinecraftSessionService"
        )

        val jarFile = inputJarFile.get().asFile
        val tmpDir = temporaryDir
        val unpacked = File(tmpDir, "unpacked")
        unpacked.deleteRecursively()
        unpacked.mkdirs()

        ant.withGroovyBuilder {
            "unzip"("src" to jarFile.absolutePath, "dest" to unpacked.absolutePath)
        }

        val commonJarOut = project(":common").tasks.named<Jar>("jar").get().archiveFile.get().asFile
        val verCommonJarOut = project(":versions:1.18.2:common").tasks.named<Jar>("jar").get().archiveFile.get().asFile

        val shadedClassInternalNames = mutableSetOf<String>()

        for (depJar in listOf(commonJarOut, verCommonJarOut)) {
            val depTmp = File(tmpDir, "dep_${depJar.name}")
            depTmp.mkdirs()
            ant.withGroovyBuilder {
                "unzip"("src" to depJar.absolutePath, "dest" to depTmp.absolutePath)
            }
            val depSrcDir = File(depTmp, srcPkg)
            val dstDir = File(unpacked, dstPkg)
            if (depSrcDir.exists()) {
                dstDir.parentFile.mkdirs()
                depSrcDir.copyRecursively(dstDir, overwrite = true)
                depSrcDir.walkTopDown()
                    .filter { it.name.endsWith(".class") }
                    .forEach { classFile ->
                        val relPath = depSrcDir.toPath().relativize(classFile.toPath()).toString()
                        val internalName = relPath.removeSuffix(".class").replace('\\', '/')
                        shadedClassInternalNames.add(internalName)
                    }
            }
        }

        logger.lifecycle("Shaded classes: $shadedClassInternalNames")

        val replacements = mutableMapOf<String, String>()

        for (internalName in shadedClassInternalNames) {
            val simpleName = internalName.substringAfterLast('/')
            val baseName = simpleName.substringBefore('$')
            if (baseName in authlibCoreSimpleNames) continue

            replacements["$srcPkg/$internalName"] = "$dstPkg/$internalName"
            val oldDot = "$srcPkg/$internalName".replace('/', '.')
            val newDot = "$dstPkg/$internalName".replace('/', '.')
            replacements[oldDot] = newDot
        }

        logger.lifecycle("Authlib replacements (${replacements.size}):")
        replacements.forEach { (k, v) -> logger.lifecycle("  $k -> $v") }

        // === SRG remapping for Forge 1.18.2 ===
        val refmapFile = File(unpacked, "versions-1.18.2-common-refmap.json")
        val mixinJsonFile = File(unpacked, "msf-friends.mixins.json")

        logger.lifecycle("Looking for refmap at: ${refmapFile.absolutePath}  exists=${refmapFile.exists()}")
        logger.lifecycle("Looking for mixin JSON at: ${mixinJsonFile.absolutePath}  exists=${mixinJsonFile.exists()}")

        // --- Locate mapping files ---
        val loomCacheDir = File(project.gradle.gradleUserHomeDir, "caches/fabric-loom/1.18.2")
        val layeredDir = loomCacheDir.listFiles()
            ?.filter { it.name.startsWith("loom.mappings.1_18_2.layered") }
            ?.maxByOrNull { it.name }
        val yarnMappingFile = if (layeredDir != null) File(layeredDir, "mappings-base.tiny") else null
        val effectiveYarnFile = if (yarnMappingFile != null && yarnMappingFile.exists()) yarnMappingFile else {
            val yarnVersion = libs.versions.yarn1182.get()
            val fallbackV2 = File(loomCacheDir, "net.fabricmc.yarn.1_18_2.${yarnVersion}-v2/mappings-base.tiny")
            val fallbackPlain = File(loomCacheDir, "net.fabricmc.yarn.1_18_2.${yarnVersion}/mappings-base.tiny")
            if (fallbackV2.exists()) fallbackV2 else fallbackPlain
        }

        val srgMappingFile = File(
            project.gradle.gradleUserHomeDir,
            "caches/minecraftforge/forgegradle/mavenizer/caches/mcp/de/oceanlabs/mcp/mcp_config" +
                "/1.18.2-20220404.173914/client/data/mappings/joined.tsrg"
        )

        logger.lifecycle("Yarn mapping file: $effectiveYarnFile (exists=${effectiveYarnFile.exists()})")
        logger.lifecycle("SRG mapping file: $srgMappingFile (exists=${srgMappingFile.exists()})")

        var mcClassInternalNames = emptySet<String>()

        if (effectiveYarnFile.exists() && srgMappingFile.exists()) {
            val srgClassByObf = mutableMapOf<String, String>()
            val srgMethodByKey = mutableMapOf<String, String>()
            val srgFieldByKey = mutableMapOf<String, String>()
            var currentObf = ""

            srgMappingFile.useLines { lines ->
                for (line in lines) {
                    if (line.startsWith("tsrg2")) continue
                    if (line.startsWith("\t")) {
                        val memberParts = line.trimStart('\t').split("\\s+".toRegex())
                        if (memberParts.size >= 3) {
                            val obfMember = memberParts[0]
                            if (obfMember == "<init>" || obfMember == "<clinit>") continue
                            if (memberParts[1].startsWith("(")) {
                                val desc = memberParts[1]
                                val srgName = memberParts[2]
                                srgMethodByKey["$currentObf|$desc|$obfMember"] = srgName
                            } else {
                                val srgName = memberParts[1]
                                srgFieldByKey["$currentObf|$obfMember"] = srgName
                            }
                        }
                    } else {
                        val classParts = line.split("\\s+".toRegex())
                        if (classParts.size >= 2) {
                            currentObf = classParts[0]
                            srgClassByObf[currentObf] = classParts[1]
                        }
                    }
                }
            }
            logger.lifecycle("Parsed SRG: ${srgClassByObf.size} classes, ${srgMethodByKey.size} methods, ${srgFieldByKey.size} fields")

            val mojangClassByObf = mutableMapOf<String, String>()
            val mojangMappingFile = rootProject.layout.projectDirectory.dir("build").file("mojang-client-1.18.2.txt").asFile
            if (mojangMappingFile.exists()) {
                mojangMappingFile.useLines { lines ->
                    for (line in lines) {
                        if (line.startsWith(" ") || line.startsWith("#")) continue
                        val arrow = line.indexOf(" -> ")
                        if (arrow < 0) continue
                        val mojangFqcn = line.substring(0, arrow).trim()
                        val obfPart = line.substring(arrow + 4).trimEnd(':')
                        val mojangInternal = mojangFqcn.replace('.', '/')
                        val obfInternal = obfPart.replace('.', '/')
                        mojangClassByObf[obfInternal] = mojangInternal
                    }
                }
                logger.lifecycle("Parsed Mojang ProGuard: ${mojangClassByObf.size} classes")
            } else {
                logger.lifecycle("WARNING: Mojang ProGuard mapping not found at ${mojangMappingFile.absolutePath}")
                // Fallback: use the official tsrg from mavenizer cache (mojang → obf)
                val officialTsrgGz = File(
                    project.gradle.gradleUserHomeDir,
                    "caches/minecraftforge/forgegradle/mavenizer/caches/mcp/de/oceanlabs/mcp/mcp_config" +
                        "/1.18.2-20220404.173914/data/mapings/official/1.18.2/official-1.18.2-obf.tsrg.gz"
                )
                if (officialTsrgGz.exists()) {
                    val gzis = GZIPInputStream(officialTsrgGz.inputStream())
                    val reader = BufferedReader(InputStreamReader(gzis))
                    reader.useLines { lines ->
                        for (line in lines) {
                            if (line.startsWith("\t")) continue
                            val parts = line.split("\\s+".toRegex())
                            if (parts.size >= 2) {
                                val mojangName = parts[0]
                                val obfName = parts[1]
                                mojangClassByObf[obfName] = mojangName
                            }
                        }
                    }
                    logger.lifecycle("Parsed official tsrg fallback: ${mojangClassByObf.size} classes")
                } else {
                    logger.lifecycle("WARNING: Official tsrg fallback not found at ${officialTsrgGz.absolutePath}")
                }
            }

            val obfToYarnClass = mutableMapOf<String, String>()
            val obfToIntClass = mutableMapOf<String, String>()
            val methodObfKeyToYarn = mutableMapOf<String, String>()
            val methodObfKeyToInt = mutableMapOf<String, String>()
            val fieldObfKeyToYarn = mutableMapOf<String, String>()
            val fieldObfKeyToInt = mutableMapOf<String, String>()
            var isV1 = false

            effectiveYarnFile.useLines { lines ->
                for (line in lines) {
                    val trimmed = line.trimStart()
                    if (trimmed.startsWith("v1")) { isV1 = true; break }
                    if (trimmed.startsWith("v2")) { isV1 = false; break }
                }
            }
            logger.lifecycle("Yarn mapping format: ${if (isV1) "v1 tiny" else "v2 tiny"}")

            if (isV1) {
                // Single pass for v1 format
                var currentObfClass = ""
                effectiveYarnFile.useLines { lines ->
                    for (line in lines) {
                        val parts = line.trimStart().split("\t")
                        if (parts.isEmpty()) continue
                        when {
                            parts[0] == "CLASS" && parts.size >= 4 -> {
                                currentObfClass = parts[1]
                                obfToYarnClass[parts[1]] = parts[3]
                                obfToIntClass[parts[1]] = parts[2]
                            }
                            parts[0] == "METHOD" && parts.size >= 6 -> {
                                val key = "${parts[1]}|${parts[2]}|${parts[3]}"
                                methodObfKeyToYarn[key] = parts[5]
                                methodObfKeyToInt[key] = parts[4]
                            }
                            parts[0] == "FIELD" && parts.size >= 6 -> {
                                val key = "${parts[1]}|${parts[3]}"
                                fieldObfKeyToYarn[key] = parts[5]
                                fieldObfKeyToInt[key] = parts[4]
                            }
                        }
                    }
                }
            } else {
                // v2 tiny: namespace order is intermediary, named, official
                // c <intermediary> <named> <official>
                //   m <desc(intermediary)> <intermediary> <named> <official>
                //   f <desc(intermediary)> <intermediary> <named> <official>
                // Desc uses intermediary class names, but SRG key needs obf names.
                // Two passes: first classes, then methods/fields.

                // Pass 1: parse all class entries
                effectiveYarnFile.useLines { lines ->
                    for (line in lines) {
                        val parts = line.trimStart().split("\t")
                        if (parts.isEmpty()) continue
                        if (parts[0] == "c" && parts.size >= 4) {
                            obfToYarnClass[parts[3]] = parts[2]  // official → named (Yarn)
                            obfToIntClass[parts[3]] = parts[1]    // official → intermediary
                        }
                    }
                }
                // Build reverse map: intermediary → official/obf class names
                val intToObfClass = mutableMapOf<String, String>()
                for ((obf, intClass) in obfToIntClass) {
                    intToObfClass[intClass] = obf
                }
                logger.lifecycle("intToObfClass.size=${intToObfClass.size}")

                // Pass 2: parse methods/fields with descriptor conversion
                var currentObfClass = ""
                effectiveYarnFile.useLines { lines ->
                    for (line in lines) {
                        val parts = line.trimStart().split("\t")
                        if (parts.isEmpty()) continue
                        when {
                            parts[0] == "c" && parts.size >= 4 -> {
                                currentObfClass = parts[3]
                            }
                            parts[0] == "m" && parts.size >= 5 -> {
                                // v2: m \t desc(intermediary) \t intermediary \t named \t official
                                // SRG key needs obf class names in desc, so convert intermediary→obf
                                val desc = parts[1].replace(Regex("L([^;]+);")) { m ->
                                    val intCls = m.groupValues[1]
                                    val obfCls = intToObfClass[intCls]
                                    if (obfCls != null) "L$obfCls;" else m.value
                                }
                                val key = "$currentObfClass|$desc|${parts[4]}" // obf|descWithObf|obfMethod
                                methodObfKeyToYarn[key] = parts[3]  // named
                                methodObfKeyToInt[key] = parts[2]    // intermediary
                            }
                            parts[0] == "f" && parts.size >= 5 -> {
                                // v2: f \t desc(intermediary) \t intermediary \t named \t official
                                val key = "$currentObfClass|${parts[4]}" // obf|obfField
                                fieldObfKeyToYarn[key] = parts[3]  // named
                                fieldObfKeyToInt[key] = parts[2]    // intermediary
                            }
                        }
                    }
                }
            }
            logger.lifecycle("Parsed Yarn: ${obfToYarnClass.size} classes, ${methodObfKeyToYarn.size} methods, ${fieldObfKeyToYarn.size} fields")

            for ((obf, yarnClass) in obfToYarnClass) {
                val mojangClass = mojangClassByObf[obf] ?: continue
                replacements[yarnClass] = mojangClass
                replacements[yarnClass.replace('/', '.')] = mojangClass.replace('/', '.')
            }
            for ((obf, intClass) in obfToIntClass) {
                val mojangClass = mojangClassByObf[obf] ?: continue
                replacements[intClass] = mojangClass
                replacements[intClass.replace('/', '.')] = mojangClass.replace('/', '.')
            }

            // Also load old Yarn names (from yarn 1.18.2+build.4 raw file) and map them to Mojang.
            // Source code may have been written against the old Yarn naming convention.
            val intToCurrentNamedClass = mutableMapOf<String, String>()
            for ((obf, intCls) in obfToIntClass) {
                val named = obfToYarnClass[obf]
                if (named != null) intToCurrentNamedClass[intCls] = named
            }
            // Build int→obf reverse map for field/method remapping
            val intToObfReverse = mutableMapOf<String, String>()
            for ((obf, intCls) in obfToIntClass) {
                intToObfReverse[intCls] = obf
            }
            val yarnRawFile = File(
                project.gradle.gradleUserHomeDir,
                "caches/fabric-loom/1.18.2/net.fabricmc.yarn.1_18_2.1.18.2+build.4/mappings-base.tiny"
            )
            if (yarnRawFile.exists()) {
                var oldYarnCount = 0
                var currentObfClassRaw = ""
                yarnRawFile.useLines { lines ->
                    for (line in lines) {
                        val parts = line.trimStart().split("\t")
                        if (parts.isEmpty()) continue
                        when {
                            parts[0] == "CLASS" && parts.size >= 4 -> {
                                currentObfClassRaw = parts[1]
                                val oldYarnNamed = parts[3]
                                val intermediary = parts[2]
                                if (oldYarnNamed != intermediary) {
                                    val currentYarn = intToCurrentNamedClass[intermediary]
                                    if (currentYarn != null && oldYarnNamed != currentYarn) {
                                        val obf = intToObfReverse[intermediary] ?: continue
                                        val mojang = mojangClassByObf[obf] ?: continue
                                        if (oldYarnNamed != mojang && !replacements.containsKey(oldYarnNamed)) {
                                            replacements[oldYarnNamed] = mojang
                                            replacements[oldYarnNamed.replace('/', '.')] = mojang.replace('/', '.')
                                            oldYarnCount++
                                        }
                                    }
                                }
                            }
                            parts[0] == "FIELD" && parts.size >= 4 -> {
                                // v1: FIELD obf intermediary named
                                val oldFieldName = parts[3]  // old Yarn field name
                                val intFieldName = parts[2]  // intermediary
                                if (oldFieldName != intFieldName && oldFieldName.length >= 6) {
                                    // Try to find SRG name via obf→SRG
                                    val obfKey = "$currentObfClassRaw|${parts[1]}"
                                    val srgName = srgFieldByKey[obfKey]
                                    if (srgName != null && oldFieldName != srgName) {
                                        replacements[oldFieldName] = srgName
                                        oldYarnCount++
                                    }
                                }
                            }
                            parts[0] == "METHOD" && parts.size >= 5 -> {
                                // v1: METHOD obf desc intermediary named
                                val oldMethodName = parts[4]
                                val intMethodName = parts[3]
                                if (oldMethodName != intMethodName && oldMethodName.length >= 6 && !oldMethodName.startsWith("<")) {
                                    val obfKey = "$currentObfClassRaw|${parts[2]}|${parts[1]}"
                                    val srgName = srgMethodByKey[obfKey]
                                    if (srgName != null && oldMethodName != srgName) {
                                        replacements[oldMethodName] = srgName
                                        oldYarnCount++
                                    }
                                }
                            }
                        }
                    }
                }
                logger.lifecycle("Old Yarn→SRG mappings added: $oldYarnCount")
            }

            // Add legacy Yarn names that aren't in any available mapping file.
            // These come from very old Yarn builds where the names differed.
            val legacyMappings = mapOf(
                "allKeys" to "f_92059_",        // obf aT, GameOptions field for KeyMapping[]
                "getConnection" to "m_6198_",   // obf a()Lpw;, ClientPacketListener method returning Connection
                "currentScreen" to "f_91080_",   // obf y, Minecraft field for current screen
                "getInstance" to "m_91087_",     // obf D()Ldyr;, Minecraft method for getInstance
                "wasPressed" to "m_90859_",      // obf f()Z, KeyMapping method consumeClick
                "width" to "f_96543_",           // obf j, Screen field width
                "height" to "f_96544_",          // obf k, Screen field height
                "addDrawableChild" to "m_142416_", // obf d(Lebd;)Lebd;, Screen method addRenderableWidget
                "screen" to "f_91080_",          // obf y, Minecraft field for current screen (current Yarn name)
                "setScreen" to "m_91153_",       // obf a(Ledw;)V, Minecraft method for setScreen
                "onPress" to "m_93750_"          // obf c(Leae;)V, Button.OnPress.press method
            )
            for ((old, srg) in legacyMappings) {
                if (!replacements.containsKey(old)) {
                    replacements[old] = srg
                }
            }
            if (legacyMappings.isNotEmpty()) {
                logger.lifecycle("Legacy Yarn→SRG mappings added: ${legacyMappings.size}")
            }

            // Handle very old Yarn names not in any mapping file (pre-build.4)
            val legacyYarnToSrg = mapOf(
                "allKeys" to "f_92059_"
            )
            for ((oldName, srgName) in legacyYarnToSrg) {
                if (!replacements.containsKey(oldName)) {
                    replacements[oldName] = srgName
                    logger.lifecycle("Legacy Yarn mapping: $oldName -> $srgName")
                }
            }
            logger.lifecycle("Legacy Yarn mappings: ${legacyYarnToSrg.size}")

            val yarnMethodToSrgMultiMap = mutableMapOf<String, MutableSet<String>>()
            for ((obfKey, intMethod) in methodObfKeyToInt) {
                val srgMethod = srgMethodByKey[obfKey] ?: continue
                if (intMethod != srgMethod) {
                    replacements[intMethod] = srgMethod
                }
                val yarnMethod = methodObfKeyToYarn[obfKey] ?: continue
                if (yarnMethod != srgMethod && yarnMethod.length >= 6) {
                    yarnMethodToSrgMultiMap.getOrPut(yarnMethod) { mutableSetOf() }.add(srgMethod)
                }
            }
            for ((yarnName, srgNames) in yarnMethodToSrgMultiMap) {
                if (srgNames.size == 1) {
                    replacements[yarnName] = srgNames.first()
                }
            }

            val yarnFieldToSrgMultiMap = mutableMapOf<String, MutableSet<String>>()
            for ((obfKey, intField) in fieldObfKeyToInt) {
                val srgField = srgFieldByKey[obfKey] ?: continue
                if (intField != srgField) {
                    replacements[intField] = srgField
                }
                val yarnField = fieldObfKeyToYarn[obfKey] ?: continue
                if (yarnField != srgField && yarnField.length >= 6) {
                    yarnFieldToSrgMultiMap.getOrPut(yarnField) { mutableSetOf() }.add(srgField)
                }
            }
            for ((yarnName, srgNames) in yarnFieldToSrgMultiMap) {
                if (srgNames.size == 1) {
                    replacements[yarnName] = srgNames.first()
                }
            }

            logger.lifecycle("SRG remapping table: ${replacements.size} total entries (Mojang classes + SRG methods/fields + authlib)")

            mcClassInternalNames = mojangClassByObf.values.toSet()

            // Convert Fabric refmap (intermediary names) to Forge format (SRG+Mojang names).
            // Fabric refmap maps: "tick" → "Lnet/minecraft/class_310;method_1574()V" (intermediary)
            // Forge refmap maps:  "tick" → "Lnet/minecraft/client/Minecraft;m_5705_()V" (SRG+Mojang)
            if (refmapFile.exists()) {
                // Build intermediary→Mojang class lookup
                val intClassToMojang = mutableMapOf<String, String>()
                for ((obf, intClass) in obfToIntClass) {
                    val mojang = mojangClassByObf[obf] ?: continue
                    intClassToMojang["L$intClass;"] = "L$mojang;"
                }
                logger.lifecycle("Refmap: intClassToMojang.size=${intClassToMojang.size}")

                // Build intermediary→SRG method lookup (unique mappings only)
                val intMethodToSrg = mutableMapOf<String, String>()
                val intMethodToSrgMulti = mutableMapOf<String, MutableSet<String>>()
                for ((obfKey, intMethod) in methodObfKeyToInt) {
                    val srg = srgMethodByKey[obfKey] ?: continue
                    if (intMethod != srg) {
                        intMethodToSrgMulti.getOrPut(intMethod) { mutableSetOf() }.add(srg)
                    }
                }
                for ((intName, srgNames) in intMethodToSrgMulti) {
                    if (srgNames.size == 1) intMethodToSrg[intName] = srgNames.first()
                }
                logger.lifecycle("Refmap: intMethodToSrg.size=${intMethodToSrg.size}")

                // Build intermediary→SRG field lookup (unique mappings only)
                val intFieldToSrg = mutableMapOf<String, String>()
                val intFieldToSrgMulti = mutableMapOf<String, MutableSet<String>>()
                for ((obfKey, intField) in fieldObfKeyToInt) {
                    val srg = srgFieldByKey[obfKey] ?: continue
                    if (intField != srg) {
                        intFieldToSrgMulti.getOrPut(intField) { mutableSetOf() }.add(srg)
                    }
                }
                for ((intName, srgNames) in intFieldToSrgMulti) {
                    if (srgNames.size == 1) intFieldToSrg[intName] = srgNames.first()
                }
                logger.lifecycle("Refmap: intFieldToSrg.size=${intFieldToSrg.size}")

                fun replaceClassRefs(str: String): String {
                    // Replace Lclass; patterns using regex to avoid substring mismatches
                    return str.replace(Regex("L([^;]+);")) { m ->
                        val intRef = m.value  // e.g. "Lnet/minecraft/class_310;"
                        intClassToMojang[intRef] ?: m.value
                    }
                }

                fun transformRefmapValue(value: String): String {
                    val semiIdx = value.indexOf(';')
                    if (semiIdx < 0) return value
                    val afterSemi = value.substring(semiIdx + 1)
                    if (afterSemi.startsWith("method_") || afterSemi.startsWith("field_")) {
                        // Method: "Lclass;method_NNNN(desc)ret"
                        val parenIdx = afterSemi.indexOf('(')
                        if (parenIdx > 0) {
                            val methodName = afterSemi.substring(0, parenIdx)
                            val srgMethod = intMethodToSrg[methodName]
                            if (srgMethod != null) {
                                val classPart = replaceClassRefs(value.substring(0, semiIdx + 1))
                                val descPart = replaceClassRefs(afterSemi.substring(parenIdx))
                                return "$classPart$srgMethod$descPart"
                            }
                        }
                        // Just transform class references using regex
                        return replaceClassRefs(value)
                    } else {
                        // Field: "field_NNNN:Lclass;"
                        val colonIdx = value.indexOf(':')
                        if (colonIdx in 0 until semiIdx) {
                            val fieldName = value.substring(0, colonIdx)
                            val srgField = intFieldToSrg[fieldName]
                            if (srgField != null) {
                                val classPart = replaceClassRefs(value.substring(colonIdx))
                                return "$srgField$classPart"
                            }
                        }
                        return replaceClassRefs(value)
                    }
                }

                val refmapText = refmapFile.readText()
                val convertedText = refmapText.replace(Regex("\"([^\"]+)\"")) { match ->
                    val str = match.groupValues[1]
                    if (str.contains(";") && (str.contains("class_") || str.contains("field_") || str.contains("method_"))) {
                        "\"${transformRefmapValue(str)}\""
                    } else {
                        match.value
                    }
                }
                refmapFile.writeText(convertedText)
                logger.lifecycle("Converted Fabric refmap to Forge format (intermediary→SRG+Mojang)")
            } else {
                logger.lifecycle("WARNING: No refmap found at ${refmapFile.absolutePath}")
            }
        } else {
            logger.lifecycle("WARNING: Could not find mapping files — SRG remapping will NOT be applied!")
        }

        val pathQualified = replacements.entries.filter {
            it.key.contains('/') || it.key.contains('.')
        }.sortedByDescending { it.key.length }
        val bareNames = replacements.entries.filter {
            !it.key.contains('/') && !it.key.contains('.')
        }.sortedByDescending { it.key.length }

        logger.lifecycle("SRG remapping: ${pathQualified.size} path-qualified, ${bareNames.size} bare-name entries")

        var patchedCount = 0
        unpacked.walkTopDown().filter { it.name.endsWith(".class") }.forEach { classFile ->
            val original = classFile.readBytes()
            val patched = patchClassConstantPool(original, pathQualified, bareNames, mcClassInternalNames, setOf("allKeys", "getConnection", "currentScreen", "getInstance", "wasPressed", "width", "height", "addDrawableChild", "screen", "setScreen", "addRenderableWidget", "press", "onPress"))
            if (!patched.contentEquals(original)) {
                classFile.writeBytes(patched)
                patchedCount++
            }
        }
        logger.lifecycle("Patched $patchedCount class files with SRG names")

        val outputFile = outputJarFile.get().asFile
        outputFile.parentFile.mkdirs()
        ant.withGroovyBuilder {
            "jar"("destfile" to outputFile.absolutePath, "basedir" to unpacked.absolutePath, "manifest" to File(unpacked, "META-INF/MANIFEST.MF").absolutePath)
        }
        logger.lifecycle("Relocated fat jar written to: $outputFile")
    }
}

fun patchClassConstantPool(
    classBytes: ByteArray,
    pathQualified: List<Map.Entry<String, String>>,
    bareNames: List<Map.Entry<String, String>>,
    mcClassNames: Set<String> = emptySet(),
    forceBareNames: Set<String> = emptySet()
): ByteArray {
    if (classBytes.size < 10) return classBytes

    var pos = 0
    pos += 4
    pos += 2
    pos += 2

    val cpCount = readU2(classBytes, pos)
    pos += 2

    data class Utf8Entry(
        val cpIndex: Int,
        val lengthFieldPos: Int,
        val dataPos: Int,
        val length: Int,
        val value: String
    )

    val utf8Indices = mutableMapOf<Int, Int>()
    val utf8Entries = mutableListOf<Utf8Entry>()
    val classRefs = mutableMapOf<Int, Int>()
    val nameAndTypeRefs = mutableMapOf<Int, Pair<Int, Int>>()
    val methodFieldRefs = mutableMapOf<Int, Pair<Int, Int>>()

    var anyPathMatch = false
    var anyBareMatch = false

    var i = 1
    var scanPos = pos
    while (i < cpCount && scanPos < classBytes.size) {
        val tag = classBytes[scanPos].toInt() and 0xFF
        scanPos += 1

        when (tag) {
            1 -> {
                val lengthFieldPos = scanPos
                val length = readU2(classBytes, scanPos)
                scanPos += 2
                val dataPos = scanPos
                if (scanPos + length > classBytes.size) break
                val utf8Str = String(classBytes.copyOfRange(scanPos, scanPos + length), Charsets.UTF_8)
                utf8Indices[i] = utf8Entries.size
                utf8Entries.add(Utf8Entry(i, lengthFieldPos, dataPos, length, utf8Str))

                if (!anyPathMatch) {
                    for ((old, _) in pathQualified) {
                        var idx2 = 0
                        while (idx2 >= 0) {
                            idx2 = utf8Str.indexOf(old, idx2)
                            if (idx2 < 0) break
                            val end2 = idx2 + old.length
                            val beforeOk = idx2 == 0 || utf8Str[idx2 - 1] == '/' || utf8Str[idx2 - 1] == '$' || utf8Str[idx2 - 1] == 'L'
                            val afterOk = end2 >= utf8Str.length || utf8Str[end2] == ';' || utf8Str[end2] == '/' || utf8Str[end2] == '$'
                            if (beforeOk && afterOk) { anyPathMatch = true; break }
                            idx2++
                        }
                        if (anyPathMatch) break
                    }
                }
                if (!anyBareMatch) {
                    for ((old, _) in bareNames) {
                        if (utf8Str.contains(old)) { anyBareMatch = true; break }
                    }
                }
                scanPos += length
            }
            3 -> { scanPos += 4 }
            4 -> { scanPos += 4 }
            5 -> { scanPos += 8; i++ }
            6 -> { scanPos += 8; i++ }
            7 -> {
                val nameIdx = readU2(classBytes, scanPos)
                classRefs[i] = nameIdx
                scanPos += 2
            }
            8 -> { scanPos += 2 }
            9 -> {
                val classIdx = readU2(classBytes, scanPos)
                val natIdx = readU2(classBytes, scanPos + 2)
                methodFieldRefs[i] = Pair(classIdx, natIdx)
                scanPos += 4
            }
            10 -> {
                val classIdx = readU2(classBytes, scanPos)
                val natIdx = readU2(classBytes, scanPos + 2)
                methodFieldRefs[i] = Pair(classIdx, natIdx)
                scanPos += 4
            }
            11 -> {
                val classIdx = readU2(classBytes, scanPos)
                val natIdx = readU2(classBytes, scanPos + 2)
                methodFieldRefs[i] = Pair(classIdx, natIdx)
                scanPos += 4
            }
            12 -> {
                val nameIdx = readU2(classBytes, scanPos)
                val descIdx = readU2(classBytes, scanPos + 2)
                nameAndTypeRefs[i] = Pair(nameIdx, descIdx)
                scanPos += 4
            }
            15 -> { scanPos += 3 }
            16 -> { scanPos += 2 }
            17 -> { scanPos += 4 }
            18 -> { scanPos += 4 }
            19 -> { scanPos += 2 }
            20 -> { scanPos += 2 }
            else -> break
        }
        i++
    }

    if (!anyPathMatch && !anyBareMatch && forceBareNames.isEmpty()) return classBytes
    if (!anyPathMatch && forceBareNames.isEmpty()) return classBytes

    val safeBareNameUtf8Positions = mutableSetOf<Int>()

    // First: add forceBareNames entries regardless of safety checks
    if (forceBareNames.isNotEmpty()) {
        for (entry in utf8Entries) {
            if (entry.value in forceBareNames) {
                safeBareNameUtf8Positions.add(entry.dataPos)
            }
        }
    }

    if (bareNames.isNotEmpty()) {
        val bareNameValues = bareNames.map { it.key }.toSet()

        for ((listIdx, entry) in utf8Entries.withIndex()) {
            if (entry.value !in bareNameValues) continue
            val utf8CpIndex = entry.cpIndex

            val referencingNATs = nameAndTypeRefs.filter { (_, nameDesc) -> nameDesc.first == utf8CpIndex }
            if (referencingNATs.isEmpty()) continue

            var allRefsAreMC = true
            var hasAnyMCRef = false
            var hasAnyRef = false

            for ((natCpIndex, _) in referencingNATs) {
                val refs = methodFieldRefs.filter { (_, classNat) -> classNat.second == natCpIndex }
                for ((_, classNatPair) in refs) {
                    hasAnyRef = true
                    val classCpIndex = classNatPair.first
                    val classNameUtf8Index = classRefs[classCpIndex]
                    if (classNameUtf8Index != null) {
                        val classNameListIdx = utf8Indices[classNameUtf8Index]
                        if (classNameListIdx != null) {
                            val className = utf8Entries[classNameListIdx].value
                            if (className in mcClassNames) {
                                hasAnyMCRef = true
                            } else {
                                allRefsAreMC = false
                            }
                        } else {
                            allRefsAreMC = false
                        }
                    } else {
                        allRefsAreMC = false
                    }
                }
            }

            // Allow replacement if ALL refs are MC (original behavior) OR if
            // there are both MC and non-MC refs (for @Shadow fields in mixin classes).
            if (hasAnyRef && (allRefsAreMC || hasAnyMCRef)) {
                safeBareNameUtf8Positions.add(entry.dataPos)
            }

            // Also allow replacement for forced bare names (legacy Yarn field names
            // like @Shadow fields that are only referenced from mixin classes).
            if (!safeBareNameUtf8Positions.contains(entry.dataPos) && entry.value in forceBareNames) {
                safeBareNameUtf8Positions.add(entry.dataPos)
            }

            // Allow replacement for forced bare names (legacy Yarn field/method names
            // like @Shadow fields/methods that are only referenced from mixin classes).
            if (!safeBareNameUtf8Positions.contains(entry.dataPos) && entry.value in forceBareNames) {
                safeBareNameUtf8Positions.add(entry.dataPos)
            }
        }
    }

    val newEntries = mutableMapOf<Int, Pair<String, ByteArray>>()
    var totalSizeDelta = 0

    for (entry in utf8Entries) {
        val original = entry.value
        data class ReplacementRange(val start: Int, val end: Int, val replacement: String)
        val ranges = mutableListOf<ReplacementRange>()

        for ((old, new_) in pathQualified) {
            var searchFrom = 0
            while (true) {
                val idx = original.indexOf(old, searchFrom)
                if (idx < 0) break
                val end = idx + old.length
                // Boundary check: only match at class name boundaries
                val beforeOk = idx == 0 || original[idx - 1] == '/' || original[idx - 1] == '$' || original[idx - 1] == 'L'
                val afterOk = end >= original.length || original[end] == ';' || original[end] == '/' || original[end] == '$'
                val overlaps = ranges.any { r -> idx < r.end && end > r.start }
                if (beforeOk && afterOk && !overlaps) {
                    ranges.add(ReplacementRange(idx, end, new_))
                }
                searchFrom = idx + 1
            }
        }

        if (ranges.isEmpty() && entry.dataPos in safeBareNameUtf8Positions) {
            for ((old, new_) in bareNames) {
                if (original == old) {
                    ranges.add(ReplacementRange(0, old.length, new_))
                    break
                }
            }
        }

        if (ranges.isNotEmpty()) {
            ranges.sortBy { it.start }
            val sb = StringBuilder()
            var lastEnd = 0
            for (range in ranges) {
                sb.append(original, lastEnd, range.start)
                sb.append(range.replacement)
                lastEnd = range.end
            }
            sb.append(original, lastEnd, original.length)
            val newStr = sb.toString()
            val newBytes = newStr.toByteArray(Charsets.UTF_8)
            totalSizeDelta += newBytes.size - entry.length
            newEntries[entry.cpIndex] = Pair(newStr, newBytes)
        }
    }

    if (newEntries.isEmpty()) return classBytes

    val newSize = classBytes.size + totalSizeDelta
    val result = ByteArray(newSize)
    var readPos = 0
    var writePos = 0

    System.arraycopy(classBytes, readPos, result, writePos, pos)
    readPos = pos
    writePos = pos

    i = 1
    while (i < cpCount && readPos < classBytes.size) {
        val tag = classBytes[readPos].toInt() and 0xFF

        when (tag) {
            1 -> {
                val oldLength = readU2(classBytes, readPos + 1)
                val newEntry = newEntries[i]
                if (newEntry != null) {
                    val newBytes = newEntry.second
                    result[writePos] = 1
                    writeU2(result, writePos + 1, newBytes.size)
                    System.arraycopy(newBytes, 0, result, writePos + 3, newBytes.size)
                    readPos += 3 + oldLength
                    writePos += 3 + newBytes.size
                } else {
                    val totalBytes = 3 + oldLength
                    System.arraycopy(classBytes, readPos, result, writePos, totalBytes)
                    readPos += totalBytes
                    writePos += totalBytes
                }
            }
            3 -> { System.arraycopy(classBytes, readPos, result, writePos, 5); readPos += 5; writePos += 5 }
            4 -> { System.arraycopy(classBytes, readPos, result, writePos, 5); readPos += 5; writePos += 5 }
            5 -> { System.arraycopy(classBytes, readPos, result, writePos, 9); readPos += 9; writePos += 9; i++ }
            6 -> { System.arraycopy(classBytes, readPos, result, writePos, 9); readPos += 9; writePos += 9; i++ }
            7 -> { System.arraycopy(classBytes, readPos, result, writePos, 3); readPos += 3; writePos += 3 }
            8 -> { System.arraycopy(classBytes, readPos, result, writePos, 3); readPos += 3; writePos += 3 }
            9, 10, 11 -> { System.arraycopy(classBytes, readPos, result, writePos, 5); readPos += 5; writePos += 5 }
            12 -> { System.arraycopy(classBytes, readPos, result, writePos, 5); readPos += 5; writePos += 5 }
            15 -> { System.arraycopy(classBytes, readPos, result, writePos, 4); readPos += 4; writePos += 4 }
            16 -> { System.arraycopy(classBytes, readPos, result, writePos, 3); readPos += 3; writePos += 3 }
            17 -> { System.arraycopy(classBytes, readPos, result, writePos, 5); readPos += 5; writePos += 5 }
            18 -> { System.arraycopy(classBytes, readPos, result, writePos, 5); readPos += 5; writePos += 5 }
            19 -> { System.arraycopy(classBytes, readPos, result, writePos, 3); readPos += 3; writePos += 3 }
            20 -> { System.arraycopy(classBytes, readPos, result, writePos, 3); readPos += 3; writePos += 3 }
            else -> {
                System.arraycopy(classBytes, readPos, result, writePos, classBytes.size - readPos)
                writePos += classBytes.size - readPos
                readPos = classBytes.size
            }
        }
        i++
    }

    if (readPos < classBytes.size) {
        System.arraycopy(classBytes, readPos, result, writePos, classBytes.size - readPos)
        writePos += classBytes.size - readPos
    }

    return if (writePos == newSize) result else result.copyOf(writePos)
}

fun readU2(bytes: ByteArray, pos: Int): Int =
    ((bytes[pos].toInt() and 0xFF) shl 8) or (bytes[pos + 1].toInt() and 0xFF)

fun writeU2(bytes: ByteArray, pos: Int, value: Int) {
    bytes[pos] = ((value shr 8) and 0xFF).toByte()
    bytes[pos + 1] = (value and 0xFF).toByte()
}
