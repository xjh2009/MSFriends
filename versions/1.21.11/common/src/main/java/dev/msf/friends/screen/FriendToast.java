    ...
    public void render(GuiGraphics graphics, Font font, long time) {
        ...
        if (description == null) {
            ...
            if (!titleLines.isEmpty()) {
                graphics.drawString(font, titleLines.getFirst(), textX, (this.height() - font.lineHeight) / 2, 0xFFFFFFFF);
            }
        } else {
            // Two lines: title + description
            int y = PADDING_TOP;
            if (!titleLines.isEmpty()) {
                graphics.drawString(font, titleLines.getFirst(), textX, y, 0xFFFFFFFF);
            }
            y += LINE_SPACING;
            List<FormattedCharSequence> descLines = font.split(description, maxTextWidth);
            if (!descLines.isEmpty()) {
                graphics.drawString(font, descLines.getFirst(), textX, y, 0xFFAAAAAA);
            }
        }
    }
