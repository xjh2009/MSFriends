...
public class FriendsScreen extends Screen {
    ...
    private static final Component SEARCH_HINT = Component.translatable("screen.msf_friends.friends.search_hint").withStyle(EditBox.SEARCH_HINT_STYLE);
    ...
    static final int PLAYERNAME_COLOR = ARGB.color(255, 255, 255, 255);
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, BACKGROUND_SPRITE, mx, 64, BG_WIDTH, this.windowHeight());
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, SEARCH_SPRITE, mx + 10, 76, 12, 12);
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        if (unselTab != null) {
            graphics.fill(unselTab.getX(), unselTab.getY(),
                    unselTab.getX() + unselTab.getWidth(),
                    unselTab.getY() + unselTab.getHeight(), 0x99000000);
        }
        // Title
        graphics.centeredText(this.font, TITLE, this.width / 2, 8, -1);
        if (this.statusMessage != null) {
            graphics.centeredText(this.font, this.statusMessage, this.width / 2, 24, this.statusColor);
        }
        // List or empty message
        if (playerList != null && !playerList.children().isEmpty()) {
            playerList.extractRenderState(graphics, mouseX, mouseY, a);
        } else {
            Component empty = hasFilter() ? EMPTY_FILTER : (page == Page.FRIENDS ? EMPTY_FRIENDS : EMPTY_PENDING);
            graphics.centeredText(this.font, empty, this.width / 2, (this.listTop() + this.listBottom()) / 2, -1);
        }
    }
    ...
    static abstract class BaseEntry extends ContainerObjectSelectionList.Entry<BaseEntry> {
        ...
        protected int renderBase(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
            ...
            // Face
            int skinX = x + 4;
            int skinY = y + (h - SKIN_SIZE) / 2;
            PlayerFaceExtractor.extractRenderState(g, skinGetter.get(), skinX, skinY, SKIN_SIZE);

            // Name
            int textX = skinX + SKIN_SIZE + 4;
            g.text(minecraft.font, playerName, textX, y + (h - minecraft.font.lineHeight) / 2, PLAYERNAME_COLOR);
    ...
    static class FriendEntry extends BaseEntry {
        ...
        private final @Nullable AbstractButton actionBtn;   // invite / join / request (icon buttons)
        private final @Nullable AbstractButton acceptBtn;   // accept incoming join request
        private final @Nullable AbstractButton rejectBtn;   // reject incoming join request
        private final @Nullable SpriteIconButton pendingLoadingBtn; // loading overlay while join request is pending
        ...
        FriendEntry(FriendsScreen screen, PlayerSocialManager.PlayerData data,
            ...
            if (pres == null) {
                ...
                // 房主已邀请我：接受邀请 = 走与「请求加入」相同的连接流程，但图标用勾
                actionBtn = null;
                ...
                // 我是房主且好友在线：显示邀请按钮，点击后保持禁用状态直到列表自然刷新
                acceptBtn = null;
                rejectBtn = null;
        ...
        @Override
        public void extractContent(GuiGraphicsExtractor g, int w, int h, boolean hovered, float delta) {
            int x = this.getContentX();
            int y = this.getContentY();
            int cw = this.getContentWidth();
            int ch = this.getContentHeight();

            // Hover highlight
            if (hovered) g.fill(x, y, x + cw, y + ch, 0x22FFFFFF);

            // Face
            int skinX = x + 4;
            int skinY = y + (ch - SKIN_SIZE) / 2;
            PlayerFaceExtractor.extractRenderState(g, skinGetter.get(), skinX, skinY, SKIN_SIZE);

            // Name (top half)
            int textX = skinX + SKIN_SIZE + 4;
            int nameY = y + ch / 3 - minecraft.font.lineHeight / 2;
            g.text(minecraft.font, playerName, textX, nameY, PLAYERNAME_COLOR);

            // Status (bottom half)
            if (status != null) {
                g.text(minecraft.font, status, textX, nameY + 12, statusColor);
            }
            ...
            btnX -= 20;
            removeBtn.setX(btnX);
            removeBtn.setY(y + (ch - 20) / 2);
            removeBtn.extractRenderState(g, 0, 0, delta);

            if (acceptBtn != null && rejectBtn != null) {
                btnX -= 22;
                rejectBtn.setX(btnX); rejectBtn.setY(y + (ch - 20) / 2);
                if (rejectBtn.visible) rejectBtn.extractRenderState(g, 0, 0, delta);
                btnX -= 22;
                if (pendingLoadingBtn != null && pendingLoadingBtn.visible) {
                    pendingLoadingBtn.setX(btnX); pendingLoadingBtn.setY(y + (ch - 20) / 2);
                    pendingLoadingBtn.extractRenderState(g, 0, 0, delta);
                } else if (acceptBtn.visible) {
                    acceptBtn.setX(btnX); acceptBtn.setY(y + (ch - 20) / 2);
                    acceptBtn.extractRenderState(g, 0, 0, delta);
                }
            } else if (actionBtn != null) {
                btnX -= actionBtn.getWidth() + 2;
                int abx = btnX, aby = y + (ch - 20) / 2;
                actionBtn.setX(abx); actionBtn.setY(aby);
                if (actionBtn.visible) actionBtn.extractRenderState(g, 0, 0, delta);
                if (pendingLoadingBtn != null) {
                    pendingLoadingBtn.setX(abx); pendingLoadingBtn.setY(aby);
                    if (pendingLoadingBtn.visible) pendingLoadingBtn.extractRenderState(g, 0, 0, delta);
                }
            }
        }
    }
    ...
    static class IncomingEntry extends BaseEntry {
        ...
        @Override
        public void extractContent(GuiGraphicsExtractor g, int w, int h, boolean hovered, float delta) {
            int textX = renderBase(g, 0, 0, delta);
            int x = this.getContentX();
            int y = this.getContentY();
            int cw = this.getContentWidth();
            int ch = this.getContentHeight();
            ...
            int btnX = x + cw - 4;
            btnX -= 20;
            declineBtn.setX(btnX); declineBtn.setY(y + (ch - 20) / 2);
            declineBtn.extractRenderState(g, 0, 0, delta);
            btnX -= 22;
            acceptBtn.setX(btnX); acceptBtn.setY(y + (ch - 20) / 2);
            acceptBtn.extractRenderState(g, 0, 0, delta);
        }
    ...
    static class OutgoingEntry extends BaseEntry {
        ...
        public void extractContent(GuiGraphicsExtractor g, int w, int h, boolean hovered, float delta) {
            int textX = renderBase(g, 0, 0, delta);
            int x = this.getContentX(), y = this.getContentY(), cw = this.getContentWidth(), ch = this.getContentHeight();
            revokeBtn.setX(x + cw - 24); revokeBtn.setY(y + (ch - 20) / 2);
            revokeBtn.extractRenderState(g, 0, 0, delta);
        }
    }
    ...
    static class JoinRequestEntry extends BaseEntry {
        ...
        public void extractContent(GuiGraphicsExtractor g, int w, int h, boolean hovered, float delta) {
            ...
            btnX -= 20;
            rejectBtn.setX(btnX); rejectBtn.setY(y + (ch - 20) / 2);
            rejectBtn.extractRenderState(g, 0, 0, delta);
            btnX -= 22;
            acceptBtn.setX(btnX); acceptBtn.setY(y + (ch - 20) / 2);
            acceptBtn.extractRenderState(g, 0, 0, delta);
        }
    ...
    static class InviteEntry extends BaseEntry {
        ...
        public void extractContent(GuiGraphicsExtractor g, int w, int h, boolean hovered, float delta) {
            ...
            int btnX = x + cw - 4;
            btnX -= 20;
            ignoreBtn.setX(btnX); ignoreBtn.setY(y + (ch - 20) / 2);
            ignoreBtn.extractRenderState(g, 0, 0, delta);
            btnX -= 22;
            joinBtn.setX(btnX); joinBtn.setY(y + (ch - 20) / 2);
            joinBtn.extractRenderState(g, 0, 0, delta);
        }
    }
}
