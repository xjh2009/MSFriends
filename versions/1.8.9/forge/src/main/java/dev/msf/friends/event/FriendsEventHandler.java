...
/**
 * Forge event handler for MC 1.8.9.
...
public final class FriendsEventHandler {
    ...
    public void onTitleScreenInit(GuiScreenEvent.InitGuiEvent.Post event) {
        ...
        if (boot != null) {
            event.buttonList.add(new GuiButton(FRIENDS_BUTTON_ID, btnX, btnY, 200, 20, FRIENDS_BUTTON_TEXT));
        }
    }
    ...
    public void onActionPerformed(GuiScreenEvent.ActionPerformedEvent.Post event) {
        if (event.button.id == FRIENDS_BUTTON_ID) {
            Minecraft mc = Minecraft.getMinecraft();
            if (mc != null) {
                mc.displayGuiScreen(new FriendsScreen(event.gui));
            }
        }
    }
}
