...
/**
 * Friends screen for MC 1.8.9.
 * Uses GuiScreen-based API (no MatrixStack, no Screen, no ButtonWidget).
 *
 * MC 1.8.9 differences from 1.19.2:
 * - GuiScreen instead of Screen
 * - GuiButton instead of ButtonWidget
 * - GlStateManager instead of RenderSystem
 * - fontRendererObj instead of textRenderer
 * - drawCenteredString/drawString instead of drawCenteredText/drawText
 * - GuiTextField instead of TextFieldWidget
 * - No MatrixStack parameter in render/draw methods
 * - ResourceLocation namespace:path constructor
 * - drawRect from GuiScreen for fills
 */
public class FriendsScreen extends GuiScreen {
    ...
    private void refreshFriendEntries() {
        ...
        // When social manager is wired up:
        // friendEntries.addAll(boot.social().getFriends().stream()
        //     .filter(f -> filter.isEmpty() || f.name().toLowerCase().contains(filter))
        //     .map(f -> new FriendEntry(f.name(), f.id()))
