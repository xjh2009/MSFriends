"""Build 1.15.2 forge and save output to file."""
import os
...
log_path = os.path.join(proj, 'build-1152-forge-compile.log')
cmd = (
    f'cd /d "{proj}" && '
    f'set "JAVA_HOME=C:\\Program Files\\Zulu\\zulu-21" && '
    f'set "JAVA_TOOL_OPTIONS=-Dfile.encoding=UTF-8" && '
    f'gradlew.bat :versions:1.15.2:forge:compileJava --no-daemon --configure-on-demand --stacktrace '
    f'> "{log_path}" 2>&1'
)
