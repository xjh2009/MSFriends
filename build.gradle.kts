...
tasks.register("collectJars") {
    ...
    doLast {
        ...
        // NeoForge 1.21.11 — relocateFatJar outputs to ${project.name}-all.jar
        val neoforge12111AllJar = project(":versions:1.21.11:neoforge").layout.buildDirectory
            .file("libs/neoforge-${project.version}-all.jar").get().asFile
        ...
        // Fabric 26.1.2
        val fabric26Jar = project(":versions:26.1.2:fabric").tasks.named<Jar>("jar").get().archiveFile.get().asFile
        val fabric26Name = "msfriends-fabric-${modVersion}+${mcVersion}.jar"
        fabric26Jar.copyTo(File(outDir.asFile, fabric26Name), overwrite = true)

        // Forge 26.1.2 — relocateFatJar outputs to ${project.name}-all.jar
        val forge26AllJar = project(":versions:26.1.2:forge").layout.buildDirectory
            .file("libs/forge-${project.version}-all.jar").get().asFile
        val forge26Name = "msfriends-forge-${modVersion}+${mcVersion}.jar"
        forge26AllJar.copyTo(File(outDir.asFile, forge26Name), overwrite = true)

        // NeoForge 26.1.2 — relocateFatJar outputs to ${project.name}-all.jar
        val neoforge26AllJar = project(":versions:26.1.2:neoforge").layout.buildDirectory
            .file("libs/neoforge-${project.version}-all.jar").get().asFile
        val neoforge26Name = "msfriends-neoforge-${modVersion}+${mcVersion}.jar"
        neoforge26AllJar.copyTo(File(outDir.asFile, neoforge26Name), overwrite = true)
